...
if __name__ == "__main__":
    ...
    # if args.mode == "single":
        # assert args.source is not None and args.source is not None
    ...
    for i, txt in enumerate(open(inputfile, 'r').read().split('\n')):
        print(i, txt)
        if args.mode == "single":
            emotions = arousals = valences = None
            ids = raw_texts = [txt[:100]]
            ...
            if preprocess_config["preprocessing"]["text"]["language"] == "kr":
                texts = np.array([preprocess_korean(txt, preprocess_config)])
            elif preprocess_config["preprocessing"]["text"]["language"] == "en":
                texts = np.array([preprocess_english(txt, preprocess_config)])
