...
from transformer.SubLayers import (
    MultiHeadAttention as BaseMultiHeadedAttention,
)
...
class MultiHeadAttention(BaseMultiHeadedAttention):
    ...
    def __init__(self, q_dim, k_dim, v_dim, n_head, n_feat, dropout_rate=0.0):
        ...
        self.d_v = self.d_k
        self.n_head = n_head
        self.w_qs = torch.nn.Linear(q_dim, n_feat)
        self.w_ks = torch.nn.Linear(k_dim, n_feat)
        self.w_vs = torch.nn.Linear(v_dim, n_feat)
        self.fc = torch.nn.Linear(n_feat, n_feat)
        self.attention = ScaledDotProductAttention(temperature=np.power(self.d_k, 0.5))
        self.dropout = torch.nn.Dropout(p=dropout_rate)

# class StyleMultiHeadAttention(torch.nn.Module):
